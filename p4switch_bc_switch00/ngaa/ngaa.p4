    ...
    apply {
        if(hdr.ngaa.isValid()) {
            ...
            if(ig_md.is_aggregation == 1 ){
                //ig_md.index = hdr.ngaa.index;//modified
                //ig_md.is_ack=hdr.ngaa.is_ack;//modified

                //frag_check.apply(hdr.ngaa.frag_id,ig_md.frag_id,ig_md); //modified
                ...
                if( hdr.ngaa.is_ack==1){ // Sent from PS.
                    ...
                    //if (ig_md.frag_id == hdr.ngaa.frag_id){ //modified
                    ig_md.index = hdr.ngaa.index; //modified
                    ig_md.aggDegree=hdr.ngaa.count //modified
                    add_count_action(); 
